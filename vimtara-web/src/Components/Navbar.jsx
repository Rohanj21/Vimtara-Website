...
export default function Navbar() {
  ...
  const timeoutRef = useRef(null); // Used to debounce the hover events
  ...
  // Map display names to their corresponding HTML IDs on the Home page
  const anchorLinks = {
    'Platform': 'platform',
    'Concierge': 'concierge',
  ...
  // GSAP Animation with Overwrite protection
  useGSAP(() => {
    if (isSolutionsOpen) {
  ...
  const handleMouseLeave = () => {
    ...
    }, 150); // 150ms buffer prevents flickering if the mouse slips
  };
  ...
  return (
    <nav className={`fixed left-0 right-0 z-50 mx-auto transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] flex items-center justify-between px-6 lg:px-8 ${isScrolled ? 'top-4 h-16 w-[95%] max-w-7xl bg-white/95 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-200 rounded-full' : 'top-8 h-20 w-[95%] max-w-7xl bg-white/5 backdrop-blur-md border border-white/10 rounded-full'}`}>
      ...
      <div className="hidden lg:flex items-center gap-8 h-full">
        {['Solutions', 'Platform', 'Concierge', 'Pricing', 'How It Works', 'Insights', 'Contact'].map((item) => {
          ...
          if (item === 'Pricing' || item === 'Contact') {
            return (
              <Link key={item} to={item === 'Pricing' ? '/pricing' : '/contact'} className={`text-sm font-semibold transition-colors duration-300 flex items-center gap-1 ${isScrolled ? 'text-slate-600 hover:text-blue-600' : 'text-slate-300 hover:text-white'}`}>
                {item}
              </Link>
            );
          }

          // 3. Anchor Links (Platform, Concierge, How It Works, Insights)
          if (anchorLinks[item]) {
            return (
              <a 
                key={item} 
                href={`/#${anchorLinks[item]}`} 
                onClick={(e) => handleAnchorClick(e, anchorLinks[item])}
                className={`text-sm font-semibold transition-colors duration-300 flex items-center gap-1 ${isScrolled ? 'text-slate-600 hover:text-blue-600' : 'text-slate-300 hover:text-white cursor-pointer'}`}
              >
                {item}
              </a>
            );
      ...
      <div className="flex items-center gap-6">
        <button className={`text-sm font-semibold hidden sm:block transition-colors duration-300 ${isScrolled ? 'text-slate-700 hover:text-blue-600' : 'text-white hover:text-blue-400'}`}>Login</button>
